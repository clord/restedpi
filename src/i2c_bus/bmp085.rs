...
impl Device {
    ...
    fn read_raw_temp(&self) -> Result<(i32, i32)> {
        self.i2c.write(self.address, Register::Control as u8, vec![Control::ReadTemp as u8])?;
    ...
    fn read_raw_pressure(&self) -> Result<i32> {
        ...
        let sampling = self.accuracy.clone() as u8;
        self.i2c.write(self.address, Register::Control as u8, vec![pressure_cmd + (sampling << 6)])?;
