...
pub fn make_token<T: serde::Serialize>(token: T, secret: &str) -> Result<String> {
    let payload = bincode::serialize(&token).map_err(|x| Error::EncodingError(format!("bincode_ser: {}", x)))?;
    ...
    let raw =  bincode::serialize(&signed).map_err(|x| Error::EncodingError(format!("bincode_ser: {}", x)))?;
...
pub fn validate_token<T: serde::de::DeserializeOwned>(token: &str, secret: &str) -> Result<T> {
    ...
    let signed_token: SignedToken = bincode::deserialize(&raw).map_err(|x| Error::EncodingError(format!("bincode_deser: {}", x)))?;
    let decoded: T = bincode::deserialize(&signed_token.payload).map_err(|x| Error::EncodingError(format!("bincode_deser: {}", x)))?;
    let secret_u8 = hex::decode(secret)?;
    let mut hmac = Hmac::new(crypto::sha2::Sha256::new(), &secret_u8);
    ...
    if hmac.result().eq(&crypto::mac::MacResult::new(&signed_token.signature)) {
        Ok(decoded)
    }
    else {
        Err(Error::EncodingError("Failed to verify signature".to_string()))
    }
}
...
mod tests {
    ...
    #[test]
    fn basic() -> Result<()> {
        let valid = SessionToken{session_version: 1, id: 1212};
        ...
        assert_eq!(validate_token::<SessionToken>(&token, "123123123123"), 
            Err(Error::EncodingError("Failed to verify signature".to_string())));
        Ok(())
    }
}
